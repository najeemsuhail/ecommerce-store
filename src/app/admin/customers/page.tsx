    ...
    if (!token) {
      ...
      const fetchCustomers = async () => {
        try {
          ...
          const url = `/api/admin/customers?${queryParams}`;
          const cacheKey = `admin-customers:${token.slice(-16)}:${url}`;
          const cachedData = getClientCache<{ data: Customer[]; totalPages: number }>(cacheKey);
          if (cachedData) {
          ...
          if (response.ok) {
            ...
            setAuthError('Failed to load customers.');
            setLoading(false);
          }
        } catch (error) {
