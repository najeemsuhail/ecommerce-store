...
interface ProductsClientPageProps {
  ...
  initialDefaultFacets: FacetData;
  initialCategoryHierarchy: CategoryHierarchyItem[];
...
export default function ProductsClientPage({
  ...
  const [facets, setFacets] = useState<FacetData>(initialFacets);
  const [defaultFacets] = useState<FacetData>(initialDefaultFacets);
  const [stableCategoryFacets] = useState<FacetData['categories']>(
    initialCategoryHierarchy.map((category) => ({
      id: category.id,
      name: category.name,
      count: 0,
    }))
  );
  const [categoryHierarchy] = useState<CategoryHierarchyItem[]>(initialCategoryHierarchy);
  ...
  const [facetFilters, setFacetFilters] = useState<FacetFilters>({
    ...
    priceRange: {
      min: initialDefaultFacets.priceRange.min > 0 ? initialDefaultFacets.priceRange.min : 0,
      max: initialDefaultFacets.priceRange.max,
    },
    attributes: {},
  });
  ...
  const buildProductsUrl = useCallback((skip: number, limit: number, includeFacets: boolean) => {
    ...
    return url;
  }, [basePriceMax, basePriceMin, facetFilters, searchTerm, sortBy]);

  const fetchFacets = useCallback(async (skip = 0) => {
